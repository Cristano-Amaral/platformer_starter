...
### Status
**Phase A implemented. Phase B implemented. Checkpoint spatial correction applied. Pending object ghosts + readable wire bounds + hybrid camera X/Y with spawn.z gameplay-lane Add + collected authored Collectible editor visibility + faded pending-delete visualization + persistent pending Add/Modify previews + viewport picking of visible pending workingCopy objects + physics-derived Platform capacity + session active↔working picking map + Delete key applied. Awaiting Phase C.**

M40 is closed. M41 is incomplete. M42 has not started.
...
Not started. Do not implement during Milestone 41.
