...
**Multi-selection and Group Translate (Milestone 78).** Transient editor state only. `additionalSelections` holds secondary members; primary remains Inspector/gizmo-pivot authority. Group Translate (two or more compatible members, Translate mode) snaps the primary intended result with M76, then applies that exact world-space delta to every member from drag-start poses. Members are not independently quantized. Checkpoint assembly, Item Pickup logical `position`, Dynamic Box authored center, and Static Prop position keep their current Translate authority. A selection containing any non-translatable member disables Group Translate and does not move a subset. Resize/Scale/Rotate/Delete/Duplicate are not group operations. `editor::EditorSelectionSet.h` and `editor::EditorGroupTranslate.h` keep the click model and shared-delta math testable without ImGui. No GUIDs, collections, parenting, Undo, CLI, Agent API, or MCP.
