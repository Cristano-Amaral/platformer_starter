...
Development **Content Browser** (F2, View menu, `workspace.showContentBrowser`) is a view over that catalog, not a second registry. Default presentation is a **Thumbnails** grid; **List** keeps the M48 name/type/path table. Search, Refresh, Import Static GLB, Add Static Prop, Place Static Prop, and confirmed Delete Asset are shared across both modes. **Add Static Prop** is the M49 direct-add UI; **Place Static Prop** is the M50 viewport placement entry (no object until a valid Ground/Platform/Slope click). `Edit > Add > Static Prop` is retained and emits the same `LevelEditorRequest::AddStaticProp`, with the menu row naming the selected asset (or `select asset`) because that Content Browser dependency is otherwise invisible in the menu bar. The selected-asset line and the Inspector Static Prop panel report when the canonical identity has no staged runtime model, naming `Build > Cook & Stage`; the classification is `editor::ClassifyStaticPropAsset` and it never triggers a cook/stage and never falls back to canonical source. Thumbnails are derived editor visualization: raylib `LoadModel` + offscreen `RenderTexture` (128×128 PNG) framed from `GetModelBoundingBox`, cached under `%LOCALAPPDATA%\Platformer3D\thumbnails\` as `{fnv1a64}.png` + `.meta` (`schema`, canonical identity, source mtime ticks, size). Cache is not source/cooked/staged authority and is not Git-tracked. Valid cache is reused; source mtime/size or schema mismatch regenerates. Generation is lazy, synchronous, at most one GLB per frame. Failures show a placeholder and retry on Refresh or source change, not every frame. View mode persists as `%LOCALAPPDATA%\Platformer3D\editor_content_browser_view.txt` (same user-data family as layout/build selection). Reset Editor Layout restores Thumbnails. Asset selection is independent of Hierarchy/viewport selection. Selecting an asset does not instantiate a scene object. While Static Prop placement is active, a new valid Content Browser identity updates the pending placement asset (Behavior A); empty/invalid selection keeps the consumed identity. Milestone 49 Delete Asset additionally refuses a canonical identity referenced by Static Props in the union of `workingCopy`, `active`, and `savedSourceBaseline` before any filesystem delete. A transient placement preview is not an authored reference. Debug has no import or Content Browser UI. Release consumes only staged runtime assets.

Development **Model Preview** (F2, View menu, `workspace.showModelPreview`, default visible like other Development panels) renders the **real** Content Browser selected source GLB through `render::StaticModelPreviewRenderer` (`LoadModel` + `RenderTexture`). It does not enlarge or depend on thumbnail PNG/cache. Shared math lives in `editor/StaticModelFraming.h` (bounds sanitize, default three-quarter frame, orbit/dolly, load classification, render-target size). Thumbnail cache identity/invalidation remains thumbnail-specific. Camera: LMB orbit (pitch clamped ±89°), wheel dolly (distance clamped from padded bounds radius); pan is omitted. **Reset View** restores default yaw/pitch/distance/target from current bounds without reloading the GLB. Resize recreates only the render target. Same identity + matching source mtime/size reuses the model; Refresh retries failures; source change reloads; delete of the selected asset clears. Lighting/background match M48.1 (`DrawModel` WHITE, `{56,60,72}`). Name/type/identity/bounds sit in a collapsed-by-default **Asset Details** `CollapsingHeader` below the interactive view (not a property inspector). Preview does not mutate authored level state or the main editor camera.
