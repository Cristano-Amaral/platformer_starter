...
**View.** Checkable items bind `workspace.showMetrics`, `showHierarchy`, `showInspector`, `showLevelEditor`. Development also binds `showObjectPalette`, `showContentBrowser`, `showModelPreview`, and `showQuickToolbar`. F1, View > Metrics, and the Metrics close button share `showMetrics`. Hierarchy/Inspector/Level Editor/Object Palette/Content Browser/Model Preview follow F2; Metrics does not. Closing a panel sets only its visibility bool (selection, workingCopy, transformMode, and pending edits remain). Hidden panels reopen from View. F2 off/on preserves session visibility and transform mode. Closing Object Palette does not cancel placement mode. Content Browser asset selection is not scene selection. Model Preview consumes that asset selection and does not change Hierarchy/viewport selection.
...
## Multiple levels / level transition (Milestone 64)

M64 is the narrowest safe `level_01 → level_02` progression. It is not a campaign graph, Level Select, SceneManager, or savegame.

**Destination.** `LevelGoalSpec::nextLevelId` is optional. Empty is a terminal M63 goal. A valid identity such as `level_02` schedules one deferred transition after M63 completion. Unsafe tokens are rejected by the same `id` grammar used for `LevelDefinition.id`.

**Authority.** Application tracks `currentRuntimeLevelId`. Initialize still requires staged `levels/level_01.level`. Destination load uses `platform::RuntimeAssetPath(MakeRuntimeLevelLogicalId(id))` only. No source/cooked fallback. File stem, requested identity, and loaded `id` must match.

**Safe point.** Completion captures the destination during the gameplay update. Destructive replace runs after `renderer.EndFrame()`, the same commit window as editor Apply/Reload, so physics/render never swap mid-draw. Release uses that same post-frame point.

**Fresh destination.** Successful replace rebuilds PhysicsWorld from the destination, then resets spawn, checkpoints, completion, collectibles, Item Pickup collected flags, M61/M62 feedback, grab/drop, Door/Plate runtime, and the run timer. Inventory carries over for the session. Restart after arrival restarts the currently loaded destination, not `level_01`. A terminal goal still shows `LEVEL COMPLETE` / `PRESS ENTER TO RESTART` with no transition.

**Failure.** Missing/malformed/unsafe destinations fail before replacing `levelDefinition` or PhysicsWorld. Completed state remains. stderr reports the failure. The failed destination is not retried every frame.

**Editor.** Inspector `Next Level` is a validated text field (empty = terminal). Duplicate copies the destination. If `workingCopy` is Modified, a runtime transition keeps those unsaved edits and does not mark them applied. If not Modified, editor authored state reinitializes to the destination without false Dirty. Save/Reload follow the current runtime identity. There is no multi-document editor.

**Content.** `game/assets/source/levels/level_02.level` is a small distinct second map cooked and staged through the existing pipeline. Canonical `level_01.level` stays fixture-free (zero Level Goals; no restored legacy `goal` or `dynamic_box 0 5 0 1 1 1 30`).
