...
**Inspector.** Lookup-by-index each frame. Platform keeps Translate/Resize numerics. Checkpoint edits Trigger Center, Trigger Size, and Respawn Position as independent authored fields. Hazard and Collectible edit `center` and `size`. Dynamic Box edits Center, Size, and Mass (kg) on `workingCopy` only. Pressure Plate edits Center and Size on `workingCopy` only; runtime Active is not an Inspector field. A collected authored Collectible remains a valid selection; Inspector still shows `workingCopy` center/size. No runtime uncollect.

**Gizmo.** Translate: Spawn, Ground, Platform, Checkpoint, Hazard, Collectible, Dynamic Box, Pressure Plate. Resize: Ground, Platform, Dynamic Box, and Pressure Plate. Checkpoint Translate is an assembly move: `SetCheckpointAssemblyCenter` applies one delta to `center` and `respawnPosition` so `respawnPosition - center` is preserved. Inspector field edits do not use that helper. No respawn gizmo. Dynamic Box gizmo edits `workingCopy` only; the live Jolt body keeps simulating until Apply. Pressure Plate gizmos edit `workingCopy` only; gameplay overlap uses `active` until Apply.
...
**Pressure Plate / Dynamic Box trigger (M52):** repeatable `std::vector<world::PressurePlateSpec>` with authored `center` and `size` only. No GUID, alias, target, action, script, or serialized Active flag. Default Add size is `{2, 0.2, 2}`; extents must be finite and `>= kMinPressurePlateExtent` (0.12). Each applied plate is a runtime AABB copied at `Initialize` / `TryRebuild`. Activation is derived on query: **Active iff at least one valid current Dynamic Box runtime AABB overlaps that authored plate AABB** (`world::PressurePlateOverlapsBox` / `AabbOverlaps`). No latch, delay, timer, overlap-set cache, or Jolt sensor body. Player, Static Props, platforms, hazards, collectibles, checkpoints, and other plates are never tested. A carried Dynamic Box (M51) activates only when its current runtime bounds overlap; carry state alone does not. Kill-plane recovery, Restart Run, checkpoint respawn, and Apply/reload recompute from the resulting box poses — no stored Active boolean survives a rebuild because none is stored. `workingCopy` edits do not affect Gameplay until Apply. Save writes `pressure_plate <cx> <cy> <cz> <sx> <sy> <sz>` only. Renderer tints Inactive `{86, 98, 124}` and Active `{56, 188, 92}`. F1 reports `Pressure Plates` and `Active Pressure Plates`. Canonical Level 01 has 0 Pressure Plates. No door, event bus, Trigger/Activator/Action/Receiver, or M53+ output.

Status: Milestone 44 is CLOSED and merged. Milestone 45 is CLOSED and merged. Milestone 46 is CLOSED and merged. Milestone 47 is CLOSED and merged. Milestone 48 / 48.1 / 48.2 are CLOSED. Milestone 49 is CLOSED. Milestone 50 is CLOSED. Milestone 51 is CLOSED. Milestone 52 is implemented, awaiting manual acceptance. Milestone 53 has not started.
