...
**Body budget.** `kPhysicsMaxBodies` remains 64. Fixed bodies are 5 (ground, 2 slopes, kinematic moving platform, CharacterVirtual inner body). Authored leftover is **59**, shared by elevated Platforms, Dynamic Boxes, and Doors: `fixed + platforms + dynamicBoxes + doors <= 64`. Canonical Level 01 static bodies stay 9 (ground + 6 platforms + 2 slopes) with 0 Dynamic Boxes and 0 Doors. Pressure Plates, Item Pickups, and Static Props never create Jolt bodies.
