...
The `LevelGoalSpec` type lives in `world/LevelGoal.h`. Authored instances live in `LevelDefinition::levelGoals` as a repeatable v1 category (`level_goal <cx> <cy> <cz> <sx> <sy> <sz>`). Canonical Level 01 has **zero** Level Goals. Runtime state lives in `gameplay::LevelCompletionState`, owned by `Application`. PhysicsWorld does not interpret goals. Renderer does not decide completion. The historical required singleton `goal` record is rejected.

Goal overlap tests `Player::Position()` (visual center) via `PlayerOverlapsAnyLevelGoal` / `PointInsideGoal` against active `levelGoals`. No Jolt sensor. Only the player can complete the level. Application calls `TryCompleteLevelFromPlayerOverlap` once per non-respawn frame after checkpoint evaluation; the first valid overlap sets `completed = true` exactly once. Remaining inside or entering another goal does not retrigger. `PerformRespawn` and PhysicsWorld rebuild do not clear or fabricate completion. Restart, Apply, and level reload reset it. Renderer draws the two-post + bar marker in Gameplay and Release; the large authored AABB is **not** part of the playable world. Development Editor draws that AABB as a restrained translucent volume (fill alpha 56, matching hidden Pressure Plate editor ghosts) plus the same marker. After `EndMode3D`, `LEVEL COMPLETE` uses raylib text in all configurations including Release. Dear ImGui Level Goal metrics remain Debug/Development only. After completion the current level stays loaded and existing player/runtime behavior continues until Enter restart.
