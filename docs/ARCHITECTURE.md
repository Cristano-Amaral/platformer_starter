...
- Level Goals: canonical Level 01 authors **zero** `level_goal` records. The historical M21 two-post instance is no longer in the file; add goals in the Development editor.
...
**Content.** `game/assets/source/levels/level_02.level` is a small distinct second map cooked and staged through the existing pipeline. Canonical `level_01.level` stays fixture-free (zero Level Goals; no restored legacy `goal` or `dynamic_box 0 5 0 1 1 1 30`). Disposable `level_03` files created during manual acceptance are not canonical unless explicitly adopted.
