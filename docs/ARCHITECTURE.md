...
**World Item Pickup (M55 / M58 / M58.3 / M58.4):** repeatable `std::vector<world::ItemPickupSpec>` with authored gameplay `position`, M54 `itemId`, `quantity`, optional canonical `modelIdentity` (`models/<file>.glb`; empty = primitive cube fallback of size `kItemPickupVisualSize` 0.5), per-instance visual-only `visualOffset`, Euler `visualRotationDegrees`, and `visualScale` (defaults `0,0,0` / `0,0,0` / `1,1,1`), presentation-only `showInteractionBounds` (default true), and presentation-only `targetHighlightIntensity` (default `0.70`, finite inclusive `[0, 1]`). Gameplay targeting, facing, LOS, Object Palette placement, and Duplicate +1 X use `position` only. Rendered model origin is `position + visualOffset`, then the same Static Prop Euler Z*Y*X and scale composition (`DrawProp` via `ItemPickupVisualProp`). No GUID, ItemDefinition, rarity, or generic Transform/Render component. Level Format: `item_pickup <px> <py> <pz> <quantity> <itemId> [visual <ox> <oy> <oz> <rx> <ry> <rz> <sx> <sy> <sz>] [bounds <0|1>] [highlight <intensity>] [<modelIdentity...>]`. Legacy M55 lines omit `visual` and receive neutral defaults. Legacy M55/M58 lines omit `bounds` and default `showInteractionBounds` true. Legacy M55/M58/M58.3 lines omit `highlight` and default `targetHighlightIntensity` 0.70. The writer always emits the `visual` marker, the explicit `bounds 0|1` marker, and the explicit `highlight` marker before `modelIdentity`. Parser reuses `gameplay::IsValidItemId` and M54 quantity range; visual scale is finite and `> 0` (Static Prop floor for the Scale gizmo); `bounds` accepts exact `0`/`1` only; `highlight` is a finite `[0, 1]` float with no silent clamp. Runtime `gameplay::ItemPickupRunState` is Application-owned `collected` flags (same size as applied `itemPickups`); **not** physics-owned and **not** serialized. NewRun / Restart / ApplyCommittedLevel rebuild flags as all available and clear Inventory (M54). Checkpoint respawn preserves Inventory **and** collected flags. `TryRebuild` alone preserves both. Targeting reuses M51 numbers: max 2.5 m, min facing dot 0.35, min distance 0.05, nearest then lower session index. LOS uses `PhysicsWorld::WorldSolidBlocksSegment` (`CastWorldRay` fraction `< 0.98`): Ground, Platforms, Slopes, moving platform, closed/moving Doors. Dynamic Boxes are ignored by that filter; Static Props have no Jolt body and do not block. Semantic `E` (`grabDropPressed` only): if carrying → Drop; else if HandleGrabDrop starts carrying → Grab consumed the press; else if a pickup target exists → `TryCollectItemPickup` which calls production `Inventory::TryAdd` **before** marking collected; else a locked Door target may unlock (M57). Failed add (including quantity-cap overflow) leaves the pickup available. Collected pickups are not rendered and not targeted. Item Pickups add no Jolt body, do not activate Pressure Plates, and cannot be grabbed. Optional `modelIdentity` participates in M49 Delete Asset protection (`workingCopy` / `active` / `savedSourceBaseline`); runtime collected does not weaken it. Assigning or clearing the Content Browser model stores only the canonical identity and does not reset the authored visual transform. Hierarchy group **Item Pickups**. Inspector: Position, Item ID, Quantity, optional Model, Visual Offset / Rotation / Scale, **Show Interaction Bounds**, and **Target Highlight Intensity** (slider `0.00..1.00`). Translate edits `position`. Rotate gizmo edits `visualRotationDegrees` at `position + visualOffset`. Scale gizmo edits `visualScale`. Visual offset remains Inspector-only. Object Palette / Edit > Add defaults `itemId=key`, `quantity=1`, empty identity, neutral visual transform, `showInteractionBounds=true`, `targetHighlightIntensity=0.70`. Scene rendering uses staged `RuntimeAssetPath` only (no Development source fallback). Missing staged GLB keeps the existing placeholder cube. Canonical Level 01 has 0 Item Pickups. Milestone 58.2 selected-model highlight for a non-empty `modelIdentity` uses `ItemPickupVisualProp(workingCopy)` so Visual Offset/Rotation/Scale and Translate of `position` all move the ghost live. Editor picking may use those transformed visual bounds; gameplay targeting does not. Empty `modelIdentity` keeps the fallback cube selection path. Milestone 58.3 Gameplay/Release target feedback consumes the existing M55 `itemPickupTargetIndex`: a model-backed uncollected target gets a depth-respecting golden tinted second pass of the same cached `StaticModelSceneStore` model (`DrawGameplayTargetHighlight`); `showInteractionBounds` then optionally draws a quieter transformed local-bounds wire. Milestone 58.4 maps `targetHighlightIntensity` to tint alpha `round(255 * intensity)` with RGB `(255, 220, 72)`; `0` skips the extra pass; `showInteractionBounds` stays independent. `showInteractionBounds = false` keeps model highlight (when intensity > 0) and `E Pick Up <itemId> x<quantity>` HUD. Renderer does not search for targets. No x-ray pass in Gameplay. No ImGui dependency.
...
Status: Milestone 44 is CLOSED and merged. Milestone 45 is CLOSED and merged. Milestone 46 is CLOSED and merged. Milestone 47 is CLOSED and merged. Milestone 48 / 48.1 / 48.2 are CLOSED. Milestone 49 is CLOSED. Milestone 50 is CLOSED. Milestone 51 is CLOSED. Milestone 52 is CLOSED. Milestone 53 is CLOSED. Milestone 54 is CLOSED. Milestone 55 is CLOSED. Milestone 56 is CLOSED. Milestone 57 is CLOSED. Milestone 57.1 is CLOSED. Milestone 58 is CLOSED. Milestone 58.1 is CLOSED. Milestone 58.2 is CLOSED. Milestone 58.3 is CLOSED. Milestone 58.4 is implemented, awaiting manual acceptance. Milestone 59 has not started.
