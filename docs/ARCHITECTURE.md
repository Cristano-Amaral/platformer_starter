...
`RestartRun()` resets physical Player/platform/cyan-box state, M20 `RespawnState` to defaults, M21 completion, M54 Inventory (cleared to empty), M57.1 Door lock flags from authored `requiredItemId`, and camera snap. On a restart frame `camera.Update` is skipped. Renderer draws `LEVEL COMPLETE` after `EndMode3D` while `levelCompleted` is true, in all configurations including Release. Terminal goals keep `PRESS ENTER TO RESTART`. Destination-bearing goals show `PRESS ENTER TO CONTINUE` during the 1.75 s runtime hold; Enter skips the remaining hold and never Restarts.
...
- Extra valid authored `source/levels/*.level` files are discovered at cook time and extra cooked `levels/*.level` files at staging time (M64.1). They are not required inventory.
...
**Fresh destination.** Successful replace rebuilds PhysicsWorld from the destination, then resets spawn, checkpoints, completion, collectibles, Item Pickup collected flags, M61/M62 feedback, grab/drop, Door/Plate runtime, the run timer, and the transition hold. Inventory carries over for the session. Restart after arrival restarts the currently loaded destination, not `level_01`. A terminal goal still shows `LEVEL COMPLETE` / `PRESS ENTER TO RESTART` with no automatic transition. A destination goal shows `LEVEL COMPLETE` / `PRESS ENTER TO CONTINUE`; Enter skips the remaining hold and never Restarts.
...
**Cook / stage.** Required inventory remains `level_01` and `level_02`. Extra valid `source/levels/*.level` files are cooked as `level_v1` and extra cooked `levels/*.level` files are staged, matching extra imported models. A Level created in the Levels UI can enter Cook & Stage without a per-level CMake list edit.
