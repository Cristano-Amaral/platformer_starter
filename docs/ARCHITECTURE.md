...
**Pressure Plate / Dynamic Box trigger (M52) / modes (M57.1):** repeatable `std::vector<world::PressurePlateSpec>` with authored `center`, `size`, optional `linkedDoorIndex` (`world::kNoLinkedDoor` / `-1` means none), `activateByDynamicBox` (default true), `activateByPlayer` (default false), and `visibleInGameplay` (default true). No GUID, alias, action, script, Trigger entity, or serialized Active flag. Default Add size is `{2, 0.2, 2}`; extents must be finite and `>= kMinPressurePlateExtent` (0.12). Each applied plate is a runtime AABB copied at `Initialize` / `TryRebuild`. Activation is derived on query: `active = boxActive || playerActive`, where `boxActive` is `activateByDynamicBox &&` any valid Dynamic Box AABB overlaps (`world::PressurePlateOverlapsBox`), and `playerActive` is `activateByPlayer &&` the CharacterVirtual capsule AABB overlaps (`PhysicsWorld` capsule radius 0.4 / total height 1.6 from feet, currently matching `kPlayerVisualSize`). Both flags false is valid and means never Active. No latch, delay, timer, overlap-set cache, or Jolt sensor body. Static Props, platforms, hazards, collectibles, checkpoints, and other plates are never tested. A carried Dynamic Box (M51) activates only when box mode is on **and** its current runtime bounds overlap. `visibleInGameplay = false` suppresses the Gameplay grey-blue/green fill; Editor mode still draws a quieter ghost so the plate remains selectable, translatable, resizable, and inspectable. Kill-plane recovery, Restart Run, checkpoint respawn, and Apply/reload recompute from the resulting poses — no stored Active boolean survives a rebuild because none is stored. `workingCopy` edits do not affect Gameplay until Apply. Parser accepts 7-token and 8-token records with legacy defaults; the writer always emits `pressure_plate <cx> <cy> <cz> <sx> <sy> <sz> <doorIndex> <box> <player> <visible>` with strict `0`/`1`. F1 reports `Pressure Plates` and `Active Pressure Plates` including invisible plates. Canonical Level 01 has 0 Pressure Plates.
