...
**Pressure Plate / Dynamic Box trigger (M52):** repeatable `std::vector<world::PressurePlateSpec>` with authored `center`, `size`, and optional `linkedDoorIndex` (`world::kNoLinkedDoor` / `-1` means none). No GUID, alias, action, script, or serialized Active flag. Default Add size is `{2, 0.2, 2}`; extents must be finite and `>= kMinPressurePlateExtent` (0.12). Each applied plate is a runtime AABB copied at `Initialize` / `TryRebuild`. Activation is derived on query: **Active iff at least one valid current Dynamic Box runtime AABB overlaps that authored plate AABB** (`world::PressurePlateOverlapsBox` / `AabbOverlaps`). No latch, delay, timer, overlap-set cache, or Jolt sensor body. Player, Static Props, platforms, hazards, collectibles, checkpoints, and other plates are never tested. A carried Dynamic Box (M51) activates only when its current runtime bounds overlap; carry state alone does not. Kill-plane recovery, Restart Run, checkpoint respawn, and Apply/reload recompute from the resulting box poses — no stored Active boolean survives a rebuild because none is stored. `workingCopy` edits do not affect Gameplay until Apply. Parser accepts 7-token no-link records; the writer always emits `pressure_plate <cx> <cy> <cz> <sx> <sy> <sz> <doorIndex>` with `-1` for none. Renderer tints Inactive `{86, 98, 124}` and Active `{56, 188, 92}`. F1 reports `Pressure Plates` and `Active Pressure Plates`. Canonical Level 01 has 0 Pressure Plates.

**Authored Door / Pressure Plate link (M53) / Key lock (M57):** repeatable `std::vector<world::DoorSpec>` with authored closed `center`, `size`, `openDistance`, and `requiresKey` (default `false`). Opening direction is fixed +Y. Default Add size is `{1.2, 3.0, 2.4}`; open distance default `3.2`, range `[0.12, 20]`. Each applied Door is one Jolt **kinematic** `BoxShape` on `ObjectLayers::Moving`. Visual greybox follows the live body. `desiredOpen` is true iff the runtime Door is **unlocked** and at least one applied Pressure Plate whose `linkedDoorIndex` names that Door is currently Active (OR; M52 remains the overlap authority). A `requiresKey` Door starts each new applied run locked (`gameplay::DoorLockRunState`, Application-owned `unlocked` flags; not serialized). Unlock consumes exactly one production Inventory item `key` via `Inventory::TryRemove` **before** marking that index unlocked. Locked Doors keep `desiredOpen = false` even if linked plates are Active. Unlocking does not itself open the Door. Motion is constant-speed `2.5 m/s` along +Y between closed and `closed + (0, openDistance, 0)` via `MoveKinematic`. Initialize / Restart snap to the derived endpoint; ordinary activation does not teleport. Closing obstruction: if `CollideShape` at the next closing pose hits the player inner body or any Dynamic Box, that closing step is paused (`blockedClosing`). Opening is never paused by that query. No crush/damage. Runtime pose and unlocked flags never mutate authored closed `center` or `requiresKey`. Save writes closed pose and `requiresKey` (`0`/`1`). Level Format: `door <cx> <cy> <cz> <sx> <sy> <sz> <openDistance> [<requiresKey>]`; 8-token records remain valid and mean false; writer always emits 9 tokens. Targeting a locked Door reuses M51/M55 numbers (2.5 m, facing dot 0.35, nearest then lower session index). LOS uses `WorldSolidBlocksSegmentIgnoringDoor` so the candidate Door body is ignored; Ground/Platform/Slope/moving platform/other Doors still block. Semantic `E` (`grabDropPressed` only): carrying → Drop; else grab target → Grab; else pickup target → Collect; else locked Door target → `TryUnlockLockedDoor`. One press is at most one action. Checkpoint respawn preserves Inventory and unlock flags. Restart / Apply / reload rebuild lock flags from authored `requiresKey` and clear Inventory. `TryRebuild` alone does not own lock flags; Application rebinds them. No extra Jolt bodies. Canonical Level 01 has 0 Doors. No generic Lock/Requirement/Interactable/dispatcher.

**Player Inventory v1 (M54):** `gameplay::Inventory` is Application-owned per-run gameplay state, sitting next to `CollectibleRunState` / `RunTimerState`. It is **not** `LevelDefinition`, **not** physics, **not** Player movement, and **not** serialized. Model: unique canonical `itemId` → positive `int` quantity (`1..kMaxItemQuantity` 1'000'000). Storage is a private `std::vector<InventoryEntry>` kept sorted by `itemId` for deterministic enumeration. Production API: `IsValidItemId`, `GetQuantity` (missing/invalid → 0), `Has`, `TryAdd`, `TryRemove`, `Clear`, `Entries()`. Failed operations do not mutate. `itemId` rules: length 1..32, first `[a-z]`, rest `[a-z0-9_-]`, case-sensitive with **no folding** (uppercase rejected). Examples `key` / `coin` / `battery` are test tokens only — there is no item catalog, ItemDefinition, icon, or `.item` format. Lifecycle (`ApplyInventoryLifecycle`): **NewRun / RestartRun / ApplyCommittedLevel (successful Apply Preview and Reload) clear** to empty; **checkpoint `PerformRespawn` and `PhysicsWorld::TryRebuild` preserve**. Inventory is not physics-owned: TryRebuild replacing Jolt state does not clear it; Application clears only through the new-run events above. M51 Grab/Carry and M52 Pressure Plates do not read or write Inventory. M57 Door unlock is the first gameplay consumer: Application calls production `TryRemove("key", 1)` before flipping that Door's runtime unlocked flag. Development F1 Metrics has an **Inventory (Test)** collapsing header (`PLATFORMER_ENABLE_LEVEL_AUTHORING` only) that calls production `TryAdd` / `TryRemove` / `Clear`. Debug has F1 Metrics without that harness. Release has no ImGui inventory UI (`GAME_RELEASE` static_assert on `kInventoryDevelopmentHarnessEnabled`). No final inventory screen, Interactable, save-game persistence, slots, hotbar, or equipment.
