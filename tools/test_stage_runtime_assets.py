...
class StageRuntimeAssetsTests(unittest.TestCase):
    ...
    def test_extra_cooked_texture_is_staged_without_required_inventory_edit(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            cooked = root / "cooked"
            dest = root / "dest"
            ...
            for relative in required:
                ...
                target.parent.mkdir(parents=True, exist_ok=True)
                shutil.copy2(repo_cooked / relative, target)
            extra = cooked / "textures" / "terrain_detail.png"
            extra.write_bytes((repo_cooked / "textures" / "test_checker.png").read_bytes())
            inventory = (REPO_ROOT / "cmake" / "RuntimeAssets.cmake").read_text(encoding="utf-8")
            self.assertNotIn("textures/terrain_detail.png", inventory)
            result = run_stage(cooked, dest, asset_list=None)
            self.assertEqual(result.returncode, 0, result.stderr + result.stdout)
