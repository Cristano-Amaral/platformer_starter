...
Release builds omit the metrics and editor panels, and F2 does nothing. Runtime assets still load from `assets/` next to the executable, including the required `levels/level_01.level`. Milestones 30–42 are complete and merged. Milestone 43 adds a Development Quick Toolbar below the menu bar (Translate/Resize, Apply/Revert/Save, persisted build selector, Run through EditorToolRunner). Debug has the M36 editor without Build/tool execution, without runtime reload, without the lifecycle Edit menu, without Object Palette, and without the Quick Toolbar. Missing or invalid Level 01 fails initialization; BEST save missing/invalid remains nonfatal. There is no LevelManager and no SceneManager.
