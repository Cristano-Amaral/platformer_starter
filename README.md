...
Known Development-tool limitations: the dirty baseline is the last successful Save in this session (seeded from the staged file at Initialize). Reload does not update that baseline. There is no filesystem watching and no automatic Cook & Stage → Reload chain. Reload is disabled while Modified (Apply or Revert first) and while Tool Output shows a running job.

Milestone 33 is complete and merged. Milestone 34 is complete and merged. Milestone 35 is complete and merged. Milestone 36 is complete and merged. Milestone 37 is complete and merged. Milestone 38 is complete and merged. Milestone 39 Phase B is implemented (awaiting Phase C). Debug omits Build and Reload Runtime Level. Do not implement Add/Delete or Milestone 40.
