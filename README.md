...
# F2 View > Content Browser        list/search registered static models;
#                           Refresh; Import Static GLB; Delete Selected Asset
#                           (project asset only; not a level edit)
```
...
Editable fields are the initial spawn, the camera offset and vertical FOV, the center/size of the ground box and of every elevated platform, and the existing authored fields of checkpoints (trigger center/size and respawn position), hazards (center/size), and collectibles (center/size). Lifecycle Add/Duplicate/Delete are Development Edit-menu only and mutate the working copy; Apply Preview, Save Level Source, and Cook, Stage & Reload remain explicit. Everything else in the file stays read-only: id, kill plane, slopes, moving platform, support indices, goal, and the cyan box.
