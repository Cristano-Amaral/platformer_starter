...
Release builds omit the metrics and editor panels, and F2 does nothing. The player-facing Inventory UI (Tab) remains available in Release; the Development Inventory (Test) harness does not. Runtime assets still load from `assets/` next to the executable, including the required `levels/level_01.level` and staged `levels/level_02.level`. Destination completion still shows `LEVEL COMPLETE` / `PRESS ENTER TO CONTINUE`, waits 1.75 s, and can skip with Enter; no ImGui is involved. Milestones 30–64 are complete. Milestone 64.1 adds the Development Levels browser/New/Open workflow and that destination hold. Canonical Level 01 has 0 Item Pickups, 0 Pressure Plates, 0 Doors, 0 Dynamic Boxes, 0 Static Props, and 0 Level Goals. Debug omits Build, Reload Runtime Level, the Edit lifecycle menu, Object Palette, Quick Toolbar, Assets import, Levels, Content Browser, Model Preview, and the Inventory test harness. Missing or invalid Level 01 fails initialization; BEST save missing/invalid remains nonfatal. There is no LevelManager, campaign graph, or SceneManager.
