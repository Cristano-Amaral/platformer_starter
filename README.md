...
Debug and Development builds also have a visual level editor toggled by F2 (`Hierarchy`, `Inspector`, and `Level Editor` windows). Opening it pauses the entire gameplay simulation: the run timer, Player, gravity, CharacterVirtual, moving platform, cyan box, Jolt stepping, fall/hazard detection, R respawn, checkpoints, goal completion, BEST comparison, collectible pickup, and Enter restart all stop. Rendering and the UI keep running. The editor camera (RMB look, WASD, Q/E, Shift, wheel speed, Alt+wheel dolly) navigates independently of the gameplay follow camera. Level Editor has Translate and Resize radios. Translate: LMB on an X/Y/Z handle moves Spawn, Ground, or an Elevated Platform in the working copy. Resize: cube handles change Ground/Platform authored size; Spawn is not resizable. Ctrl+Arrows/PageUp/PageDown nudge in Translate mode only. A screen-space orientation widget sets canonical editor views (yaw/pitch only). LMB elsewhere picks a world object. Hierarchy rows select the same identity. Unapplied gizmo/Inspector edits show a cyan ghost; the active object, physics, picking, and yellow highlight stay put until Apply Preview. F2 again resumes gameplay (follow camera snaps to the Player). F2 is ignored while an editor text field owns the keyboard, so typing a value cannot close the editor. Clicks, drags, and wheel on ImGui panels do not pick, drag a gizmo, or navigate the world behind them. **Reset Editor Layout** in Level Editor restores Metrics, Hierarchy, Inspector, and Level Editor to the project defaults.

Release builds omit the metrics and editor panels, and F2 does nothing. Runtime assets still load from `assets/` next to the executable, including the required `levels/level_01.level`. Milestones 30–34 are complete and merged. Milestone 35 Phase B (live resize gizmo, orientation widget, nudge, Alt+wheel dolly) is implemented and awaiting Phase C. Do not treat M35 as complete. Missing or invalid Level 01 fails initialization; BEST save missing/invalid remains nonfatal. There is no LevelManager and no SceneManager.
...
Milestone 33 is complete and merged. Milestone 34 is complete and merged (translation gizmo, pending ghost, persistent layout, overlay depth). Milestone 35 Phase B wires live Translate/Resize, the orientation widget, Translate-only keyboard nudge, and Alt+wheel dolly. Do not implement rotation, add/delete, or Milestone 36.
