...
The Development executable opens a resizable 1280x720 window titled `Platformer3D` at the Main Menu (`Platformer3D`, **PLAY**, **QUIT**; Up/Down select, Enter activate). Gameplay simulation and the run timer do not advance until PLAY. PLAY starts a fresh staged `level_01` run; QUIT requests the normal window close. A frozen 3D backdrop may already be prepared from staged Level 01 during Initialize. After PLAY, use A/Left or D/Right to move along X, and Space or Up Arrow to jump. Press **Tab** to open or close the player Inventory (read-only list of owned `itemId` / quantity; Up/Left previous, Down/Right next; Esc also closes). While Inventory is open, gameplay simulation pauses so movement, jump, Grab/Drop, and Item Pickup collection do not fire; a carried Dynamic Box is not dropped. Press **Esc** during ordinary Gameplay to open the Pause Menu (**RESUME** / **MAIN MENU**); Esc while paused resumes. Press R to respawn at the current spawn (initial spawn, Checkpoint 1 after it activates, or Checkpoint 2 after that). Falling below the level kill plane (Y = -8 in Level 01) respawns and increments death count (Manual R does not). Touching a static Hazard deals **25** Health on the first overlapping simulation step, then at most once every **1.0 s** of simulation time until the player leaves; leaving and re-entering damages immediately again. Health is **100** maximum, shown as `HEALTH current / max` below the objective HUD during active Gameplay. Successful Hazard damage shows a brief red edge vignette (**0.35 s**, peak opacity **0.45**) and a one-shot damage sound. Reaching 0 Health enters one death phase (`YOU DIED`, **1.25 s**, one death sound), then respawns at the active Checkpoint or current-Level spawn with full Health and one respawn sound, without reloading the Level or resetting the run. Pause, Inventory, and F2 cannot open or age that feedback incorrectly during death; F2 is ignored until respawn completes. Fall/Manual R still respawn without restoring Health and without the Health-death respawn sound. Pause, Inventory, F2, Main Menu, destination `LEVEL COMPLETE`, and `RUN COMPLETE` apply no Hazard damage. Two red/orange spike bars sit on the ground: one in the center ↔ Checkpoint 1 corridor, one in the Checkpoint 2 → goal gap. They are non-solid gameplay volumes, not physics bodies. Three gold cubes are optional hop collectibles (right platform, left landing, middle-left step). A `TIME MM:SS.mmm` readout sits in the upper-left in all configurations, with `BEST --:--.---` or `BEST MM:SS.mmm` directly below it, a compact `LEVEL 01` / `OBJECTIVE: REACH THE LEVEL GOAL` pair below BEST during active Gameplay (Level 02 uses `LEVEL 02` / `OBJECTIVE: REACH THE FINAL LEVEL GOAL`), `HEALTH 100 / 100` below the objective line, and a `COLLECTED N / <level collectible count>` counter sits in the upper-right (Level 01 still has 3). The objective HUD and Health HUD are hidden on the Main Menu, while Inventory is open, during F2, during Pause, during destination `LEVEL COMPLETE`, during `RUN COMPLETE`, and during the `YOU DIED` death phase. Time uses the gameplay frame delta, freezes on the first level completion, stays frozen through R / fall and leftover collectibles, and resets to `00:00.000` only when Enter starts a fresh run. Session BEST is the fastest completed run: first completion sets it, a faster later run replaces it, a slower run or exact tie leaves it, and Enter preserves it. A valid BEST is persisted under `%LOCALAPPDATA%\Platformer3D\best_time_v1.txt` and restored on the next launch; missing or invalid saves show `BEST --:--.---`. Collected items stay collected through R and fall; Enter after level complete restores them to 0 / 3. Runtime Inventory (M54) is likewise preserved through R / fall and cleared on Enter Restart Run; it is not a world collectible and is not saved with the level. Collectibles do not gate the goal. Two post+beacon markers show ordered checkpoints: steel when future, bright green when current, darker green when previously activated. Checkpoint 2 cannot activate before Checkpoint 1; backtracking does not downgrade progress. The far-left platform still exists as geometry. Canonical Level 01 authors **one** destination-bearing Level Goal to `level_02`; Level 02 authors **one** terminal Level Goal. Reaching any active Level Goal volume with the player completes the level (`LEVEL COMPLETE`). A destination-bearing goal (`Next Level: level_02`) shows `PRESS ENTER TO CONTINUE` for **1.75 seconds**, then loads staged `levels/level_02.level` once and **preserves current Health**; Enter skips the remaining hold and never Restarts. After a successful transition, Enter restarts the currently loaded destination, not `level_01`, and restores Health to maximum. A missing or invalid destination keeps the completed current level and Health unchanged and is not retried every frame. Completing the terminal Level 02 Goal shows `RUN COMPLETE` with the captured `TIME`: Enter is Play Again (fresh staged `level_01` at full Health), R restarts the current Level at full Health, and Esc returns to Main Menu without starting a run. From Pause, **MAIN MENU** returns to the existing Main Menu; the next PLAY starts full Health on staged `level_01`. Enter before completion does nothing. The camera follows with dead zones and smoothing, and snaps immediately on respawn and restart. Player collision and physical position come from Jolt CharacterVirtual. Gameplay draws a project-owned staged `models/player.glb` placeholder that follows that authoritative visual-center; offset/scale/yaw are presentation-only. The old orange AABB remains only if the staged model cannot load. The player is not an authored Level object. Canonical Level 01 authored data lives in `game/assets/source/levels/level_01.level`. The cooker copies it to `game/assets/cooked/levels/level_01.level`; CMake stages `<exe>/assets/levels/level_01.level`. Application loads that staged file once in Initialize into `LevelDefinition`. Renderer and PhysicsWorld derive visuals and bodies from the loaded definition. There is no compiled Level 01 fallback. After Checkpoint 1 the Player returns toward the center on open ground. A 30-degree static slope past Checkpoint 1 is an optional walkable test; a 60-degree static slope further right is steep/non-walkable. One Jolt kinematic platform moves back and forth on X; standing on it carries the Player. The Level Format `dynamic_box` record is a repeatable authored Dynamic Box (center, size, mass in kg). Canonical Level 01 has zero Dynamic Boxes so the old cyan probe is not scene clutter. Place boxes in the Development editor, then Apply Preview. Jolt pose is runtime authority; Save writes authored centers. Full Enter restart restores authored poses and zeros velocities; checkpoint respawn does not. Press **E** to Grab / Drop a nearby Dynamic Box in front of the player (2.5 m max, blocked by solid Ground/Platform/Slope/Door geometry). If no box is carried and no grab target exists, the same **E** collects a nearby available Item Pickup (`E Pick Up <itemId> x<qty>`). A successful collect also plays the M61 burst/sound and shows a short `Picked Up <itemId> x<qty>` HUD confirmation above that prompt. If neither applies, **E** can unlock a nearby locked Door that requires a `key` (`E Unlock Door (key)` / `Requires key`). Carry, collection, and unlock are runtime-only and never mark the authored level Modified. A `pressure_plate` record is a repeatable authored trigger volume (center, size). It is Active while at least one runtime Dynamic Box overlaps that AABB, Inactive with none, and never serializes Active/Inactive. Pressure Plates do not consume Jolt body slots. Canonical Level 01 has zero Pressure Plates. If a live Dynamic Box center falls below the authored kill plane (Y = -8 in Level 01), only that box returns to its currently applied authored transform with identity orientation and zero velocity; other boxes keep their runtime state. A carried box that falls through the kill plane is released, then recovered the same way. Cooker test assets (`textures/test_checker.png`, `models/test_static.glb`, `models/test_authored.glb`, `models/test_textured.glb`) remain in cook/stage inventory and automated tests; Milestone 44 does not load or draw them in Level 01. Debug and Development builds show a `Platformer3D Metrics` panel (F1 toggles it), including Level Loading, Level Data, Run timer, Session best, Persistence, Respawn / Checkpoint, Hazards (including Health), Collectibles, and Level Goal. Development adds an **Inventory (Test)** section in that same F1 panel: inspect `itemId` / quantity and manually Add / Remove / Clear through the production Inventory API (not the player HUD; Tab opens the separate game Inventory UI). From Main Menu, QUIT or the window close button exits through the normal shutdown path. During Gameplay, Esc opens Pause unless Inventory is open, Run Complete is showing, destination `LEVEL COMPLETE` owns input, or the Main Menu is active.

Debug and Development builds also have a visual level editor toggled by F2 (`Hierarchy`, `Inspector`, and `Level Editor` windows, plus a top Dear ImGui menu bar: View / Transform / Level). Development adds an Edit menu (Add Platform/Checkpoint/Hazard/Collectible/Dynamic Box/Pressure Plate/Door/Item Pickup/Level Goal/Static Prop, Duplicate Selected, Delete Selected, Group Selected, Ungroup; with a multi-selection those Duplicate/Delete commands copy or remove the complete selected set; Group Selected creates one persistent Authoring Group; Ungroup removes group metadata only; Delete key matches Delete Selected and is ignored while ImGui owns the keyboard), an Object Palette with viewport placement for Platform/Checkpoint/Hazard/Collectible/Dynamic Box/Pressure Plate/Door/Item Pickup/Level Goal (not Static Prop), a compact Quick Toolbar below the menu bar (Translate, Resize, Scale, Rotate, Apply, Revert, Save, build selector, Run), an Assets menu (Import Static GLB), a **Levels** window (View > Levels; discovered authored source levels, current identity, Open, New Level, Refresh; Dirty/Modified Open/New asks Cancel vs Discard), a Content Browser (View > Content Browser; catalog of registered static models, search, Refresh, Import Static GLB, Add Static Prop, confirmed Delete Asset), an **Item Database** (View > Item Database; reusable `items/<name>` definitions, Save/Reload of `gameplay/definitions.gameplay`, existing model/texture pickers; not Inventory), a Model Preview (View > Model Preview; real selected GLB with orbit/zoom/Reset View), a Build menu (Cook Assets, Stage Runtime Assets, Cook & Stage, Cook, Stage & Reload, Build Debug/Development/Release, Build All) and a Tool Output window. Opening the editor pauses the entire gameplay simulation: the run timer, Player, gravity, CharacterVirtual, moving platform, Jolt stepping, fall detection, Hazard Health damage, R respawn, checkpoints, goal completion, BEST comparison, collectible pickup, and Enter restart all stop. Rendering and the UI keep running. The editor camera (RMB look, WASD, Q/E, Shift, wheel speed, Alt+wheel dolly) navigates independently of the gameplay follow camera. Level Editor, Transform menu, and Quick Toolbar share Translate, Resize, Scale, and Rotate. Translate: LMB on an X/Y/Z handle moves Spawn, Ground, an Elevated Platform, Checkpoint, Hazard, Collectible, Dynamic Box, Pressure Plate, Door, Item Pickup, Level Goal, or Static Prop in the working copy. Resize: cube handles change Ground/Platform/Dynamic Box/Pressure Plate/Door/Level Goal authored size; Spawn/Checkpoint/Hazard/Collectible/Item Pickup/Static Prop are not resizable. Scale: cube handles change Static Prop `scale` or Item Pickup `visualScale`. Rotate: world-axis X/Y/Z rings change Static Prop authored rotation or Item Pickup `visualRotationDegrees` (gizmo origin is `position + visualOffset` for pickups). Item Pickup Translate still edits gameplay `position`; visual offset stays Inspector-only. Ctrl+Arrows/PageUp/PageDown nudge in Translate mode only. A screen-space orientation widget sets canonical editor views (yaw/pitch only) and sits below the menu bar and Quick Toolbar. LMB elsewhere picks a world object. Hierarchy lists persistent Authoring Groups first, then ungrouped objects; grouped members appear only under their group. Click a group row to select the complete composition, or a member row to select that object. Hierarchy Group Selected / Rename / Ungroup sit above the tree. Ordinary click replaces selection; Ctrl+click adds or removes authored objects, with the most recently chosen object as primary (yellow) and secondaries in orange. Ctrl-clicking a group row still selects that complete group. With two or more translatable objects selected, Translate moves the set by one shared snapped delta from the primary gizmo. With two or more currently rotatable objects selected (Static Prop / Item Pickup), Rotate orbits secondaries around the PRIMARY world-axis gizmo and applies one shared snapped orientation delta; Resize/Scale stay single-object and are disabled while multi-selected. Duplicate Selected / Delete Selected operate on the complete selection. Ctrl while dragging a gizmo still inverts Snap. Unapplied gizmo/Inspector edits show a cyan ghost; the active object, physics, picking, and yellow highlight stay put until Apply Preview except for model-backed Static Props and Item Pickups, whose selected-model highlight follows `workingCopy` live. Pending deletes remain visible in the viewport with a distinct wireframe until Apply. F2 again restores the prior top-level flow (Main Menu stays Main Menu; Gameplay resumes with the follow camera snapped to the Player). F2 from Main Menu does not start a run. Authored Open/Switch remains Development-only and does not silently turn Main Menu into Gameplay. F2 is ignored while an editor text field owns the keyboard, so typing a value cannot close the editor. Clicks, drags, and wheel on ImGui panels, the menu bar, or the Quick Toolbar do not pick, drag a gizmo, or navigate the world behind them. **Reset Editor Layout** (Level menu or Level Editor button) restores Metrics, Hierarchy, Inspector, Level Editor, Object Palette, Levels, Content Browser, Model Preview, Item Database, Tool Output, and Quick Toolbar visibility to the project defaults, snaps those windows, and restores Content Browser **Thumbnails** view. It does not reset the last toolbar build configuration. F1 and View > Metrics toggle the same Metrics visibility; Metrics can stay open with F2 off.
...
Release builds omit the metrics and editor panels, and F2 does nothing. The player-facing Main Menu, PLAY/QUIT, Pause Menu (Esc), Inventory UI (Tab), Gameplay Level/objective HUD, Health HUD, Damage Vignette, `YOU DIED` death phase, pickup/damage/death/respawn SFX, destination hold, and Run Complete results remain available in Release; the Development Inventory (Test) harness does not. Runtime assets still load from `assets/` next to the executable, including the required `levels/level_01.level` and staged `levels/level_02.level`. All runtime Level loads stay staged-only. Destination completion still shows `LEVEL COMPLETE` / `PRESS ENTER TO CONTINUE`, waits 1.75 s, and can skip with Enter; terminal results add `ESC     MAIN MENU`; no ImGui is involved. Canonical Level 01 has 0 Item Pickups, 0 Pressure Plates, 0 Doors, 0 Dynamic Boxes, 0 Static Props, and one destination Level Goal to `level_02`. Canonical Level 02 is the run finale and authors a Checkpoint, Hazard, Collectible, Key Item Pickup, key Door, Dynamic Box, visible box-activated Pressure Plate, plate Door, and terminal Level Goal. Debug omits Build, Reload Runtime Level, the Edit lifecycle menu, Object Palette, Quick Toolbar, Assets import, Levels, Content Browser, Model Preview, and the Inventory test harness. Missing or invalid Level 01 fails initialization; BEST save missing/invalid remains nonfatal. There is no LevelManager, campaign graph, SceneManager, Settings, Continue, or Level Select.
...
# Hierarchy group row   select the complete Authoring Group
# Hierarchy member row  select only that object
# Hierarchy Group Selected / Rename / Ungroup   working copy only when they mutate groups
# Edit > Add / Duplicate / Delete / Group Selected / Ungroup   working copy only; "Modified" turns true
...
# F2 View > Levels                  discovered authored source levels;
#                           current identity; Open; New Level; Refresh.
#                           Dirty/Modified Open/New asks Cancel vs Discard.
...
.\build\windows-vs2022\Development\PlayerCharacterStatsTest.exe # M102 Player base/equipment/effective stats
...
.\build\windows-vs2022\Development\EditorGizmoTest.exe      # M34 translation + M35 resize/nudge math + M76 snapping
.\build\windows-vs2022\Development\EditorViewportGridTest.exe # M77 viewport-grid spacing / lines / layout
...
.\build\windows-vs2022\Development\AuthoredObjectLifecycleTest.exe # M41/M79 working-copy Add/Duplicate/Delete
.\build\windows-vs2022\Development\AuthoredLifecycleIntegrationTest.exe # M41/M79 owner-side lifecycle
...
.\build\windows-vs2022\Development\ModelMaterialPresentationTest.exe # M84 Base Color / texture presentation
.\build\windows-vs2022\Development\LoadedModelMaterialsTest.exe # M84 ownership / highlight restore
...
- Milestone 18: `models/test_textured.glb` embeds (or must embed) its Base Color; `test_textured_basecolor.png` is authoring-only and is not a runtime asset. Milestone 84 uses this as the project-owned textured reference (embedded Base Color Texture only). See `docs/BLENDER_WORKFLOW.md`.
...
- Milestone 61/71/72/73/74/75 gameplay SFX: `sounds/item_pickup_collect.wav`, `sounds/player_damage.wav`, `sounds/player_death.wav`, `sounds/player_respawn.wav`, `sounds/player_footstep.wav`, `sounds/player_jump.wav`, `sounds/player_land.wav`, `sounds/checkpoint_activate.wav`, `sounds/pressure_plate_activate.wav`, `sounds/pressure_plate_deactivate.wav`, `sounds/door_unlock.wav`, `sounds/level_goal_complete.wav`, `sounds/collectible_collect.wav`, `sounds/ui_navigate.wav`, `sounds/ui_confirm.wav`, `sounds/pause_open.wav`, `sounds/pause_close.wav`, `sounds/inventory_open.wav`, `sounds/inventory_close.wav`. Project-owned PCM WAVs; cook/stage; runtime never reads `source/`.
