...
Release builds omit the metrics and editor panels, and F2 does nothing. Runtime assets still load from `assets/` next to the executable, including the required `levels/level_01.level`. Milestones 30–37 are complete and merged. Milestone 38 Phase B adds Development `Build > Stage Runtime Assets` and `Build > Cook & Stage` using `cmake -P cmake/StageRuntimeAssets.cmake` (Development dest only; no C++ build). Debug has the M36 editor without Build/tool execution. Missing or invalid Level 01 fails initialization; BEST save missing/invalid remains nonfatal. There is no LevelManager and no SceneManager.
...
Milestone 33 is complete and merged. Milestone 34 is complete and merged. Milestone 35 is complete and merged. Milestone 36 is complete and merged. Milestone 37 is complete and merged. Milestone 38 Phase B is awaiting Phase C. Debug omits Build. Do not implement Add/Delete or Milestone 39.
