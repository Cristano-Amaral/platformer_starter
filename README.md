...
Release builds omit the metrics and editor panels, and F2 does nothing. Runtime assets still load from `assets/` next to the executable, including the required `levels/level_01.level`. Milestones 30–39 are complete and merged. Milestone 40 Phase B adds Development `Build > Cook, Stage & Reload` (canonical Cook & Stage, then one in-process M39 reload). Debug has the M36 editor without Build/tool execution and without runtime reload. Missing or invalid Level 01 fails initialization; BEST save missing/invalid remains nonfatal. There is no LevelManager and no SceneManager.
...
Milestone 33 is complete and merged. Milestone 34 is complete and merged. Milestone 35 is complete and merged. Milestone 36 is complete and merged. Milestone 37 is complete and merged. Milestone 38 is complete and merged. Milestone 39 is complete and merged. Milestone 40 Phase B is implemented (awaiting Phase C). Debug omits Build and Reload Runtime Level. Do not implement Add/Delete or Milestone 41.
