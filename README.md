...
Debug and Development builds also have a visual level editor toggled by F2 (`Hierarchy`, `Inspector`, and `Level Editor` windows, plus a top Dear ImGui menu bar: View / Transform / Level). Development adds a Build menu (Cook Assets, Build Debug/Development/Release, Build All) and a Tool Output window. Opening the editor pauses the entire gameplay simulation: the run timer, Player, gravity, CharacterVirtual, moving platform, cyan box, Jolt stepping, fall/hazard detection, R respawn, checkpoints, goal completion, BEST comparison, collectible pickup, and Enter restart all stop. Rendering and the UI keep running. The editor camera (RMB look, WASD, Q/E, Shift, wheel speed, Alt+wheel dolly) navigates independently of the gameplay follow camera. Level Editor and Transform menu share Translate and Resize. Translate: LMB on an X/Y/Z handle moves Spawn, Ground, or an Elevated Platform in the working copy. Resize: cube handles change Ground/Platform authored size; Spawn is not resizable. Ctrl+Arrows/PageUp/PageDown nudge in Translate mode only. A screen-space orientation widget sets canonical editor views (yaw/pitch only) and sits below the menu bar. LMB elsewhere picks a world object. Hierarchy rows select the same identity. Unapplied gizmo/Inspector edits show a cyan ghost; the active object, physics, picking, and yellow highlight stay put until Apply Preview. F2 again resumes gameplay (follow camera snaps to the Player). F2 is ignored while an editor text field owns the keyboard, so typing a value cannot close the editor. Clicks, drags, and wheel on ImGui panels or the menu bar do not pick, drag a gizmo, or navigate the world behind them. **Reset Editor Layout** (Level menu or Level Editor button) restores Metrics, Hierarchy, Inspector, Level Editor, and Tool Output to the project defaults and shows all five panels. F1 and View > Metrics toggle the same Metrics visibility; Metrics can stay open with F2 off.

Release builds omit the metrics and editor panels, and F2 does nothing. Runtime assets still load from `assets/` next to the executable, including the required `levels/level_01.level`. Milestones 30–36 are complete and merged. Milestone 37 (Development Build menu and Tool Output) is implemented and Phase C validated; Git closure is still pending. Debug has the M36 editor without Build/tool execution. Missing or invalid Level 01 fails initialization; BEST save missing/invalid remains nonfatal. There is no LevelManager and no SceneManager.
...
cmake --build --preset windows-development                # may be a C++ no-op;
                                                          # POST_BUILD still stages
                                                          # cooked -> runtime assets
.\build\windows-vs2022\bin\Development\Platformer3D.exe   # relaunch to load staged copy
```

After an authored change, a **fresh restart** test is: Apply Preview → Save Level Source → Cook Assets → Build Development → Restart Development. These steps are independent. Save is not Cook. Cook is not runtime staging. Build Development is still `cmake --build --preset windows-development`; it is not a dedicated "stage assets" command, but the existing CMake POST_BUILD copies `game/assets/cooked/` into `build/windows-vs2022/bin/<Config>/assets/`. The C++ executable does not need to recompile when only level/asset data changed, and its timestamp may stay unchanged. Development `Build > Cook Assets` is only `python tools/cook_assets.py` (no stage, no build, no restart).
...
Milestone 33 is complete and merged. Milestone 34 is complete and merged. Milestone 35 is complete and merged. Milestone 36 is complete and merged. Milestone 37 is implemented and Phase C validated (Git closure pending). Debug omits Build. Do not implement Add/Delete or Milestone 38.
...
.\build\windows-vs2022\Development\EditorToolRunnerTest.exe # M37 process capture + job/Build All policy
