...
Release builds omit the metrics panel. Runtime assets still load from `assets/` next to the executable, including the required `levels/level_01.level`. Milestone 30 (`LevelDefinition`) is complete and merged. Milestone 31 Phase B (external level file v1 live migration) is implementation-complete and awaiting Phase C manual validation. Missing or invalid Level 01 fails initialization; BEST save missing/invalid remains nonfatal. There is no level editor or LevelManager. Do not treat M31 as complete.
...
- Milestone 31 live Level 01: `levels/level_01.level`. Format: `docs/LEVEL_FORMAT_V1.md`. Edit `game/assets/source/levels/level_01.level`, then cook/build. Do not edit cooked or staged copies.
